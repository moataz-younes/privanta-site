...
path = Path(__file__).resolve().parents[1] / "src/pages/platform/PlatformContent.tsx"
text = path.read_text(encoding="utf-8")

# Keep lines 1-173 (imports + data + SectionIntro)
head = text.split("function FeatureCard")[0]

tail = '''function FeatureCard({
  icon: Icon,
  title,
  desc,
}: {
  ...
  return (
    <article className="platform-card">
      <div className="platform-card-icon">
        <Icon className="h-4 w-4 text-[var(--teal)]" strokeWidth={2} aria-hidden />
      </div>
...
# Fix placeholders: motionIconWrap in tail is wrong - write correct tail without typos
correct_tail = tail.replace("motionIconWrap", "MOTION_PLACEHOLDER")
# Now I need to write correct JSX manually in the file - use a dict of replacements

replacements = {
    "MOTION_PLACEHOLDER": "div",  # will break - do structured instead
}

path.write_text(head + open(__file__).read().split("CORRECT_JSX = '''")[1].split("'''")[0], encoding="utf-8")
